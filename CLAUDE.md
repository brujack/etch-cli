...
│   └── src/
│       ├── actions/          # 48 action types (see Action Catalog below)
...
48 actions — full field reference in [`docs/knowledge/action-catalog.md`](docs/knowledge/action-catalog.md).
...
Unit tests in `lib/src/`, integration tests in `app/tests/` (assert_cmd + insta snapshots). Coverage ~86% macOS / ~81.33% Linux CI — gap is macOS-only tests gated with `#[cfg(target_os = "macos")]`.
