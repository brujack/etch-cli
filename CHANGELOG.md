...
- atomic restore and skip orphaned stash files (#113)

- preserve original file permissions on restore (#114)
...
- document DEBCONF_NONINTERACTIVE_SEEN and NEEDRESTART_MODE in apt troubleshooting

- document apt env var propagation in package example

- pointer stub per ADR-0020 (#112)

- bump Linux coverage 81.01% → 81.84% after rollback permission fix (#114)

- rollback preserves file permissions; bump coverage 81.84% → 82.64%

- add bug-scan to Phase 3 chain

- bump macOS coverage figure to 86.47%

- remove ntfy notification from backlog
...
- add powershell.module action (#116)



## Refactoring

- adopt canonical .claude/memory + .claude/retrospectives layout (ADR-0014) (#110)
...
- replace flaky ignored test with wiremock mock (#115)
