...
| 2026-05-29 | —                                                                                        | [state-manifest](specs/2026-05-29-state-manifest-design.md)                                                               | Pending |
| 2026-05-29 | —                                                                                        | [version-pinning](specs/2026-05-29-version-pinning-design.md)                                                             | Pending |
...
| Feature                                    | Notes                                                                                                                                                                                                                                                                                                                                                                          |
| ------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| ruby.install compile_flags (macOS)         | `ruby.install` has no field for passing custom compiler flags. On macOS, OpenSSL is keg-only so ruby-install needs `-- --with-openssl-dir=$(brew --prefix openssl@3)` to link correctly. Add optional `compile_flags: Vec<String>` (or `extra_args`) passed through to `ruby-install` after `--`. Until implemented, `studio/ruby.yaml` must use `command.run`.                |
| macos.rosetta                              | No action for ensuring Rosetta 2 is installed on Apple Silicon. Needs `macos.rosetta` with no fields, idempotent via `arch -x86_64 true` exit code, runs `softwareupdate --install-rosetta --agree-to-license`. macOS-only; gate with `where: 'os.name == "macos"'`.                                                                                                           |
...
| verbose apply output                       | Most actions log generic messages (`nothing to be done to reconcile action`, `Installing packages`) with no context about what is being checked or acted on — file paths, package names, command being run. Each action log line should include the relevant subject (path, package name, command) so the operator can follow what etch is doing without reading the manifest. |
...
| coverage: etch status command              | `app/src/commands/status.rs` is 0% covered (78 lines). `run_status()` never tested — needs integration tests spawning `etch status` binary, checking exit code and stdout structure. Biggest single coverage gap on Linux CI.                                                                                                                                                  |
| coverage: etch plugin list/remove          | `app/src/commands/plugin.rs` is 0% covered (22 lines). The `list` and `remove` subcommand paths are untested. Add CLI integration tests via `assert_cmd` covering: list with no plugins installed, remove non-existent plugin (error), remove installed plugin.                                                                                                                |
...
| coverage: actions/mod.rs variant condition | `lib/src/actions/mod.rs` at 79% (33 lines). Uncovered: variant `condition` evaluation branch and the false-condition early-return path. Add unit test with an `Actions` node whose `condition` evaluates to `false` — verify action is skipped.                                                                                                                                |
| coverage: file/copy.rs privileged path     | `lib/src/actions/file/copy.rs` at 79% (29 lines). Uncovered: the `privileged: true` execution path that wraps the copy atom with privilege escalation. Add integration test with a `file.copy` action where `privileged: true` (requires sudo in CI — may need to mock the atom or accept the gap).                                                                            |
| coverage: file/link.rs glob privileged     | `lib/src/actions/file/link.rs` at 83% (23 lines). Uncovered: the `glob + privileged: true` combination branch. Add unit test for a `file.link` with `glob` pattern and `privileged: true` — verify the privileged atom is invoked for each matched file.                                                                                                                       |
| coverage: config/mod.rs error paths        | `app/src/config/mod.rs` at 73% (15 lines). Uncovered: `find_configs()` error branch when config dir is inaccessible, and the unrecognised config format error. Add tests using a temp dir with no read permission and a config file with unsupported extension.                                                                                                                |
| coverage: atoms/file/remove.rs             | `lib/src/atoms/file/remove.rs` at 64% (11 lines). Uncovered: the parent-metadata error path (stat fails before remove) and the case where the file was already removed between plan and execute. Add tests injecting a path whose parent dir is inaccessible.                                                                                                                  |
| coverage: atoms/file/chmod.rs              | `lib/src/atoms/file/chmod.rs` at 77% (9 lines). Uncovered: `stat()` failure path (file disappears between plan and execute). Add test that removes the file after planning but before execution.                                                                                                                                                                               |
| coverage: systemd/service.rs error cases   | `lib/src/actions/systemd/service.rs` at 60% (9 lines). Uncovered: the privileged=true execution path and the error case where `systemctl` returns non-zero. Add unit tests mocking `systemctl` to fail; verify error propagation. Linux-only — gate tests with `#[cfg(target_os = "linux")]`.                                                                                  |
