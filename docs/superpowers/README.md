...
| macos.default array-add and delete | `macos.default` only supports scalar `write`; `-array-add` (e.g. `menuExtras`) and `defaults delete` not supported. Add `operation: write\|delete` field and `array_add: true` mode. Source comment at `lib/src/actions/macos/default.rs:8` explicitly notes arrays were deferred.     |
| macos.service (launchctl)          | No action for enabling/disabling macOS daemons. Dotfiles use `launchctl load -w <plist>` for SSH server and ARD. Needs `macos.service` action with `plist:`, `state: loaded\|unloaded`, `privileged: true`. Idempotency via `launchctl list` exit code.                                |
...
| Handler/notify pattern             | No way to run an action only when a preceding action made a change (e.g. `killall SystemUIServer` after `macos.default` changes). Ansible-style handlers: actions declare `notify: <handler-name>`; handlers run once at end of manifest if notified.                                  |
| git.pull / git.clone update flag   | `git.clone` is provision-once (skips if directory exists). No way to keep cloned repos current. Add `update: true` field to `git.clone` (runs `git pull` if repo exists) or a dedicated `git.pull` action with `directory:` and `rebase: bool`.                                        |
