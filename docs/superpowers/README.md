...
| journald layer swallows DEBUG logs      | On Linux, `configure_tracing` adds a journald layer but DEBUG/TRACE messages never appear in journald — only INFO and above. Root cause: the `stdout_writer.with_max_level(INFO)` filter likely propagates as the global filter, suppressing lower levels before they reach the journald layer. Fix: set an explicit per-layer filter on journald so it receives all levels independently of the stdout writer. |
