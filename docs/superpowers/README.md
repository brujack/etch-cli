...
| manifest-examples          | Expand `examples/` to comprehensively cover every action: one file per option variant (e.g. `file/link-glob.yaml`, `file/chmod-privileged.yaml`), inline comments on every field, and a top-level `examples/kitchen-sink.yaml` combining all action types in a realistic manifest. Currently many actions have no example or only the simplest case. Goal: a human can learn to write any manifest correctly from examples alone, without reading source code. |
