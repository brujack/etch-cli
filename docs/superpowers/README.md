...
| Benchmark regression alerts       | Add `comment-on-alert: true` + `alert-threshold: "130%"` to `benchmarks.yml`; gh-pages history already accumulating                                                                                                              |
| `cargo semver-checks`             | Catch unintentional breaking API changes in `etch-lib` before release; one CI step in the release workflow                                                                                                                       |
| Snapshot testing (`insta`)        | Lock CLI stdout/stderr format; unexpected output changes surface as visible diffs rather than silent regressions                                                                                                                 |
| `cargo deny` license policy       | Add license allowlist to catch GPL-contaminating transitive deps; advisory CVE scanning already present                                                                                                                          |
