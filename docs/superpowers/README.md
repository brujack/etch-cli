...
| package.autoremove                         | No way to run `apt autoremove` to clean up orphaned dependencies after package removal. Needs a `package.autoremove` action (or `provider: apt` option on an existing action) that runs `apt autoremove -y` with `DEBIAN_FRONTEND=noninteractive`. Gate with `where: 'os.family == "linux"'`. Idempotency: always runs (apt autoremove is safe to call when nothing to remove). |
