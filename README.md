...
| `powershell.module`                                                                                                         | Install PowerShell modules from PSGallery (idempotent). `name` (single module) or `list` (multiple — mutually exclusive); `scope` (`CurrentUser` default or `AllUsers` — AllUsers requires elevated privileges). Requires `pwsh` in PATH.                                                                                                                                                                   |
...
- `pip install -r requirements-ci-test.txt` — Python lint and test toolchain, supplying both
  ...
  dev-venv package set, kept byte-identical to its source in dotfiles
